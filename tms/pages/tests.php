<?php
$user = getCurrentUser();
$institute_id = isset($user['institute_id']) ? $user['institute_id'] : null;
    ...
    if ($action === 'create' || $action === 'edit') {
        try {
            ...
            if ($action === 'create') {
                ...
                $stmt->execute([$title, $description, $price, $mrp, $test_type, $timer_type, $duration, $institute_id, $is_active]);
...
try {
    if ($user['role'] === 'super_admin') {
        $stmt = $db->prepare("SELECT * FROM test_packs ORDER BY created_at DESC");
        $stmt->execute();
    } else {
        $stmt = $db->prepare("SELECT * FROM test_packs WHERE institute_id = ? ORDER BY created_at DESC");
