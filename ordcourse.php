                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <table class="table table-bordered" border='1' width="100%">
                                    ...
                                    <tr>
                                        <th>Remarks</th>
                                        <td><?php if ($row['remark'] == '') { ?><span style="color:red;">Not Processed
                                                    Yet</span><?php } else {
                                            echo $row['remark'];
                                        } ?></td>
                                    </tr>
...
if (isset($_SESSION['affiliate_ref']) && isset($_SESSION['affiliate_product'])) {
    ...
    $order_id = $row['id']; // course order/subscription id
    ...
    if ($referrer_row = mysqli_fetch_assoc($referrer_query)) {
        ...
        if ($referrer_user_id != $buyer_user_id) {
            if (preg_match('/^(course)_(\\d+)$/', $product, $pmatch)) {
                ...
                if (mysqli_num_rows($exists) == 0) {
                    ...
                    $rowc = mysqli_fetch_assoc(mysqli_query($con, "SELECT csprice, affiliate_commission_percent FROM course WHERE id='$product_id'"));
                    $commission_percent = isset($rowc['affiliate_commission_percent']) ? $rowc['affiliate_commission_percent'] : 10;
                    $commission_amount = round($rowc['csprice'] * ($commission_percent / 100), 2);
